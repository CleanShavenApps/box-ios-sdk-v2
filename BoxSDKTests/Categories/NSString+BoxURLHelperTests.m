...
- (void)testThatNonURLEncodedStringsHaveContentsPreserved
{
    ...
    XCTAssertEqualObjects(stringWithURLEncodeableCharacters, nonEncodedString, @"String contents should be preserved when not url encoding");
}
...
- (void)testThatAllURLEncodeableCharactersAreURLEncoded
{
    ...
    XCTAssertEqualObjects(expectedEncodedString, actualEncodedString, @"Actual url encoded string (%@) did not match expected (%@)", actualEncodedString, expectedEncodedString);
}
