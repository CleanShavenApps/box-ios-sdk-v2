...
{
    XCTAssertEqualObjects(@"Rick Astley", user.name, @"expected name did not match actual");
}

- (void)testThatNameIsReturnedAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.name, @"expected status should be nil");
}

- (void)testThatLoginIsReturnedFromFullFormat
{
    XCTAssertEqualObjects(@"rick@astley.com", user.login, @"expected login did not match actual");
}

- (void)testThatLoginIsReturnedAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.login, @"expected status should be nil");
}
...
- (void)testThatCreatedAtIsParsedCorrectlyIntoADateFromFullFormat
{
    ...
    XCTAssertEqualObjects(expectedDate, user.createdAt, @"expected created_at did not match actual");
}

- (void)testThatCreatedAtIsReturnedAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.createdAt, @"created_at should be nil");
}
...
- (void)testThatCreatedAtIsReturnedAsNilIfSetToAGarbageValue
{
    ...
    XCTAssertNil(garbageUser.createdAt, @"created_at should be nil when user is initalized with a garbage value");
}
...
- (void)testThatModifiedAtIsParsedCorrectlyIntoADateFromFullFormat
{
    ...
    XCTAssertEqualObjects(expectedDate, user.modifiedAt, @"expected modified at did not match actual");
}

- (void)testThatModifiedAtIsReturnedAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.modifiedAt, @"modified at should be nil");
}
...
- (void)testThatModifiedAtIsReturnedAsNilIfSetToAGarbageValue
{
    ...
    XCTAssertNil(garbageUser.modifiedAt, @"modified at should be nil when user is initalized with a garbage value");
}

- (void)testThatRoleIsReturnedFromFullFormat
{
    XCTAssertEqualObjects(@"admin", user.role, @"expected role did not match actual");
}

- (void)testThatRoleIsReturnedAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.role, @"expected role should be nil");
}

- (void)testThatLanguageIsReturnedFromFullFormat
{
    XCTAssertEqualObjects(@"en", user.language, @"expected language did not match actual");
}

- (void)testThatLanguageIsReturnedAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.language, @"expected role should be nil");
}

- (void)testThatSpaceAmountIsReturnedFromFullFormat
{
    XCTAssertEqualObjects(@10737418240, user.spaceAmount, @"expected space_amount did not match actual");
}

- (void)testThatSpaceAmountIsReturnedAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.spaceAmount, @"expected space_amount should be nil");
}

- (void)testThatSpaceUsedIsReturnedFromFullFormat
{
    XCTAssertEqualObjects(@5368709120, user.spaceUsed, @"expected space_used did not match actual");
}

- (void)testThatSpaceUsedIsReturnedAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.spaceUsed, @"expected space_used should be nil");
}

- (void)testThatMaxUploadSizeIsReturnedFromFullFormat
{
    XCTAssertEqualObjects(@1073741824, user.maxUploadSize, @"expected max_upload_size did not match actual");
}

- (void)testThatMaxUploadSizeIsReturnedAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.maxUploadSize, @"expected max_upload_size should be nil");
}
...
- (void)testThatTrackingCodesIsReturnedFromFullFormat
{
    ...
    XCTAssertEqualObjects(expectedTrackingCodes, user.trackingCodes, @"expected tracking_codes did not match actual");
}

- (void)testThatTrackingCodesisReturnedAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.trackingCodes, @"expected trackingCodes should be nil");
}

- (void)testThatCanSeeManagedUsersIsParsedCorrectlyIntoABooleanFromFullFormat
{
    XCTAssertEqual(@(YES), user.canSeeManagedUsers, @"expected can_see_managed_users should be set to True");
}

- (void)testThatCanSeeManagedUsersIsSetAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.canSeeManagedUsers, @"expected can_see_managed_users should be nil");
}

- (void)testThatIsSyncEnabledIsParsedCorrectlyIntoABooleanFromFullFormat
{
    XCTAssertEqual(@(NO), user.isSyncEnabled, @"expected is_sync_enabled should be set to False");
}

- (void)testThatIsSyncEnabledIsSetAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.isSyncEnabled, @"expected is_sync_enabled should be nil");
}

- (void)testThatStatusIsReturnedFromFullFormat
{
    XCTAssertEqualObjects(@"active", user.status, @"expected status did not match actual");
}

- (void)testThatStatusIsReturnedAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.status, @"expected status should be nil");
}

- (void)testThatJobTitleIsReturnedFromFullFormat
{
    XCTAssertEqualObjects(@"Singer", user.jobTitle, @"expected job_title did not match actual");
}

- (void)testThatJobTitleIsReturnedAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.jobTitle, @"expected job_title should be nil");
}

- (void)testThatPhoneIsReturnedFromFullFormat
{
    XCTAssertEqualObjects(@"1234567890", user.phone, @"expected phone did not match actual");
}

- (void)testThatPhoneIsReturnedAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.phone, @"expected phone should be nil");
}

- (void)testThatAddressIsReturnedFromFullFormat
{
    XCTAssertEqualObjects(@"Never Gonna Let You Down Blvd", user.address, @"expected address did not match actual");
}

- (void)testThatAddressIsReturnedAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.address, @"expected address should be nil");
}

- (void)testThatIsExemptFromDeviceLimitsIsParsedCorrectlyIntoABooleanFromFullFormat
{
    XCTAssertEqual(@(YES), user.isExemptFromDeviceLimits, @"expected is_exempt_from_device_limits should be set to True");
}

- (void)testThatIsExemptFromDeviceLimitsIsSetAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.isExemptFromDeviceLimits, @"expected is_exempt_from_device_limits should be nil");
}

- (void)testThatIsExemptFromLoginVerificationIsParsedCorrectlyIntoABooleanFromFullFormat
{
    XCTAssertEqual(@(NO), user.isExemptFromLoginVerification, @"expected is_exempt_from_login_verification should be set to False");
}

- (void)testThatIsExemptFromLoginVerificationIsSetAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.isExemptFromLoginVerification, @"expected is_exempt_from_login_verification should be nil");
}
...
- (void)testThatAvatarURLIsParsedCorrectlyIntoAURLFromFullFormat
{
    ...
    XCTAssertEqualObjects(expectedAvatarURL, user.avatarURL, @"expected avatar_url should be set to False");
}

- (void)testThatAvatarURLIsSetAsNilIfUnsetFromMiniFormat
{
    XCTAssertNil(miniUser.avatarURL, @"expected avatar_url should be nil");
}
