...
- (void)testThatSDKURLAndOAuth2URLAreKeptInSyncBySetter
{
    ...
    // precondition
    XCTAssertFalse([SDK.OAuth2Session.APIBaseURLString isEqualToString:APIBaseURL], @"OAuth2 base URL is not different from test target URL");

    SDK.APIBaseURL = APIBaseURL;

    XCTAssertEqualObjects(SDK.APIBaseURL, SDK.OAuth2Session.APIBaseURLString, @"Shared SDK and OAuth2 base URLs are not in sync");

}

- (void)testThatSDKAndOAuth2SessionAreConstructedWithSameAPIUrl
{
    XCTAssertEqual([BoxSDK sharedSDK].APIBaseURL, [BoxSDK sharedSDK].OAuth2Session.APIBaseURLString, @"BoxSDK and OAuth2Session were not constructed with the same API base URL");
}
...
- (void)testThatSingletonSDKIsOnlyInstantiatedOnce
{
    ...
    XCTAssertTrue(firstSingletonSDK == secondSingletonSDK, @"multiple invocations of [BoxSDK sharedSDK] should refer to the same object");
}
